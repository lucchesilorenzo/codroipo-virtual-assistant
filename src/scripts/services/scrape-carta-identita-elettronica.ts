...
export async function scrapeCartaIdentitaElettronica() {
  ...
  const cartaIdentitaElettronica: CartaIdentitaElettronica =
    await page.evaluate(() => {
      ...
      if (areaServizioEl) {
        ...
        if (cardContattiEl) {
          ...
          const pecSpan = spans.find((span) =>
            span.textContent.includes("PEC")
          );
          if (pecSpan) {
  ...
  return cartaIdentitaElettronica;
}
